...
const BigTile: React.FC<{ eyebrow: string; title: string; sub?: string }> = ({
  eyebrow,
  title,
  sub,
}) => (
  <div className="w-full bg-gray-900 rounded-3xl ring-1 ring-white/10 p-8 md:p-12 text-center">
    <div className="text-white uppercase tracking-wider text-sm md:text-base mb-3">{eyebrow}</div>
    <div className="text-4xl md:text-6xl font-extrabold text-white leading-tight">{title}</div>
    {sub ? <div className="text-gray-400 text-base md:text-lg mt-4">{sub}</div> : null}
  </div>
);
...
function InvestorsGate({ onGranted }: { onGranted: () => void }) {
  ...
  async function onSubmit(event: React.FormEvent) {
    ...
    if (!form.agreeNda) {
      return setError("You must accept the confidentiality statement.");
    }
    ...
    return (
      <main className="pt-20 container mx-auto px-6 py-12">
        <SectionHero
          ...
          subtitle="We have sent a 6-digit code to verify your access."
        ...
        <div className="max-w-2xl mx-auto mt-8 bg-[#0b0b0b] p-6 rounded-2xl ring-1 ring-white/10">
          ...
          <input
            ...
            pattern="[0-9]*"
            maxLength={6}
            className="w-full rounded-xl bg-black/40 border border-white/10 px-4 py-3 text-white tracking-[0.4em] text-center text-xl focus:outline-none focus:ring-2 focus:ring-white/30"
            placeholder="000000"
            value={code}
          ...
          <div className="mt-6 flex flex-wrap items-center gap-3">
            {!verified ? (
              <>
                <button
                  onClick={onVerify}
                  className="px-5 py-3 rounded-xl bg-black text-white font-semibold hover:bg-black/90 disabled:opacity-50"
                  disabled={verifying}
                >
                ...
                <button
                  onClick={onResend}
                  className="px-5 py-3 rounded-xl border border-white/20 text-white hover:bg-white/5 disabled:opacity-50"
                  disabled={submitting}
                >
                  Resend code
                </button>
  ...
  return (
    <main className="pt-20 container mx-auto px-6 py-12">
      <SectionHero
        ...
        subtitle="A short overview for interested investors, with detailed materials available on request."
        heightClass="h-44 md:h-56"
        greenOpacity={0.25}
      />

      <div className="max-w-3xl mx-auto mt-8 space-y-6">
        <div className="rounded-2xl bg-[#0b0b0b] p-6 md:p-8 ring-1 ring-white/10">
          <h2 className="text-2xl font-bold text-white">For prospective investors</h2>
          <p className="mt-4 text-gray-300 leading-relaxed">
            Vitalos is building an AI-powered mental healthcare platform focused on
            earlier intervention, safer triage, and scalable support for health
            systems. Investor materials are available on request for qualified
            conversations.
          </p>
          <div className="mt-6 flex flex-wrap items-center gap-3">
            <button
              ...
              className="px-5 py-3 rounded-xl bg-black text-white font-semibold hover:bg-black/90"
            >
              Request Access
            </button>
            {canSkip ? (
        ...
        {showRequestForm ? (
          <form onSubmit={onSubmit} className="bg-[#0b0b0b] p-6 rounded-2xl ring-1 ring-white/10">
            <h3 className="text-xl font-semibold text-white">Request investor access</h3>
            <p className="mt-2 text-sm leading-7 text-gray-400">
              Complete the short form below and we will email you a 6-digit code to verify access.
            </p>
            ...
            <div className="mt-6 grid grid-cols-1 md:grid-cols-2 gap-4">
              <div>
                ...
                <input
                  className="w-full rounded-xl bg-black/40 border border-white/10 px-3 py-2 text-white focus:outline-none focus:ring-2 focus:ring-white/30"
                  value={form.fullName}
            ...
            <div className="mt-4 space-y-3">
              <label className="flex items-start gap-3 text-sm text-gray-300">
                <input
                  ...
                  onChange={(event) => onChange("agreeNda", event.target.checked)}
                />
                <span>
                  I acknowledge this page contains confidential information and agree not to
                  share or redistribute without written consent.
                </span>
              </label>
              <label className="flex items-start gap-3 text-sm text-gray-400">
                ...
                />
                <span>I am happy for Vitalos to contact me about this request.</span>
              </label>
            </div>
    ...
    <div className="max-w-6xl mx-auto mt-6">
      <div className="flex items-center justify-between mb-4">
        ...
        <div className="flex gap-2">
          <button
            onClick={prev}
            className="px-4 py-2 rounded-lg border border-white/15 text-white hover:bg-white/5 disabled:opacity-40"
            disabled={index === 0}
          >
            Back
          </button>
          <button
            onClick={next}
            className="px-4 py-2 rounded-lg bg-black text-white font-semibold hover:bg-black/90 disabled:opacity-40"
        ...
        {slides.map((_, slideIndex) => (
          <button
            ...
            className={`h-2 rounded-full transition-all ${
              slideIndex === index ? "w-8 bg-black" : "w-2 bg-white/20 hover:bg-white/30"
            }`}
          />
        ))}
      </div>
...
function InvestorsSlides() {
  ...
  const slides: React.ReactNode[] = [
    <div key="overview" className="px-1">
      <motion.div
      ...
      >
        ...
        <motion.div variants={item}>
          <BigTile eyebrow="Model" title="B2B SaaS" sub="Per-seat and per-population" />
        </motion.div>
        <motion.div variants={item}>
          <BigTile eyebrow="Pipeline" title="NHS and partners" sub="Pilots plus councils, HE, OH" />
        </motion.div>
      </motion.div>
    </div>,

    <div key="problem-mission" className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <Card title="Problem">
        Services are overwhelmed: long waits, risk spotted late, and high admin burden.
        Tools are either basic trackers or opaque models that do not fit care pathways.
      </Card>
      <Card title="Mission">
        Make mental healthcare earlier, fairer, and scalable with clinical-grade AI
        that fits real-world pathways, is privacy-first, and is interoperable by design.
      </Card>
    </div>,

    <div key="ask" className="bg-gray-900 rounded-2xl p-6 md:p-8 ring-1 ring-white/10">
      <h3 className="text-2xl md:text-3xl font-bold text-white">The Ask</h3>
      <p className="text-gray-300 mt-3 text-lg">
        We are raising <span className="text-white font-semibold">GBP X.Xm</span> to fund 18-24 months of execution.
      </p>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mt-6">
        <BigTile eyebrow="Budget" title="40%" sub="Engineering, core product, safety, interop" />
        <BigTile eyebrow="Budget" title="25%" sub="Clinical and data, advisors, evaluations, governance" />
        <BigTile eyebrow="Budget" title="35%" sub="GTM, pilots, procurement, partners" />
      </div>
    </div>,

    <div key="contact" className="max-w-3xl mx-auto">
      <Card title="Contact">
        <p>
          Email{" "}
          <a href="mailto:investors@vitalos.co.uk" className="text-white">
            investors@vitalos.co.uk
          </a>{" "}
          or call 0203 432 3640.
        </p>
      </Card>
    </div>,
  ];
  ...
  return (
    <main className="pt-20 container mx-auto px-6 pb-16">
      ...
      <p className="text-center text-xs text-gray-500 mt-10">
        Copyright {new Date().getFullYear()} Vitalos Technologies
      </p>
    </main>
  );
}
